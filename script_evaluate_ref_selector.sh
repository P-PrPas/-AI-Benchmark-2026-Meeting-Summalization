...
conda run -n "$CONDA_ENV_NAME" python -u -m finetune.evaluate_ref_selector \
  "$@" \
  ...
  --selector-model-path "$OUTPUT_DIR/ref_selector.pkl" \
  --output-path "$OUTPUT_DIR/ref_selector.eval.json" \
